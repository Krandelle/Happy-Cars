						...
						<table class="table table-condensed table-bordered table-hover">
							<thead>
								<tr>
									...
									<th>Borrower</th>
									<th>Valid ID</th> <!-- ✅ NEW COLUMN HEADER -->
									<th>Date Pick-up</th>
									<th>Date Drop-off</th>
									<th>Car Info</th>
									<th>Status</th>
							...
							<tbody>
								...
								while($row = $qry->fetch_assoc()){ $cat[$row['id']] = $row['name']; }

								$qry = $conn->query("SELECT * FROM transmission_types ");
								while($row = $qry->fetch_assoc()){ $tt[$row['id']] = $row['name']; }

								$qry = $conn->query("SELECT * FROM engine_types ");
								while($row = $qry->fetch_assoc()){ $et[$row['id']] = $row['name']; }

								$books = $conn->query("SELECT b.*,c.model,c.brand,c.transmission_id,c.engine_id FROM books b 
								...
								<tr>
									...
									<td>
										<?php if (!empty($row['valid_id_path']) && file_exists($row['valid_id_path'])): ?>
											<img src="<?php echo $row['valid_id_path'] ?>" alt="Valid ID" style="max-width: 100px; max-height: 100px;">
										<?php else: ?>
											<small>No ID</small>
										<?php endif; ?>
									</td>

									<td><p><b><?php echo date("M d,Y h:i A",strtotime($row['pickup_datetime'])) ?></b></p></td>
									<td><p><b><?php echo date("M d,Y h:i A",strtotime($row['dropoff_datetime'])) ?></b></p></td>
